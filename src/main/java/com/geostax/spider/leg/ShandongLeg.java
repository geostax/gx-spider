...
public class ShandongLeg implements Runnable{
	...
	public ShandongLeg(String leg_id, DomainSpider spider, boolean feed) {
		this.leg_id = leg_id;
		this.spider = spider;
	...
	public void run() {
		while (true) {
			...
			try {
				...
				if (!doc.select("meta[name=pubDate]").isEmpty()
						&& !doc.select("meta[name=pubDate]").first().attr("content").toString().equals("")) {
	...
	public void parse(Document doc) {
		...
		IndexResponse response = client.prepareIndex("shandong", "page").setSource(infoMap).execute().actionGet();
